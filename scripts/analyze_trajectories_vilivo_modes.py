...
#rc('font', **{'family': 'serif', 'serif': ['Times New Roman Bold']})
    ...
    for dataset_idx, dataset_nm in enumerate(dataset_names):
        ...
        for alg in p_es_0:
            pu.plot_trajectory_top(ax, p_es_0[alg], COLORS[alg],
                                   PLOT_LABELS[alg])
        pu.plot_trajectory_top(ax, p_gt_0, 'm', 'Ground Truth')
        plt.legend(bbox_to_anchor=(0, 1.03, 1, 1.5), loc=8, ncol=2, borderaxespad=0.,fontsize='medium')
        ...
        for alg in p_es_0:
            pu.plot_trajectory_side(ax, p_es_0[alg], COLORS[alg], 
                                    PLOT_LABELS[alg])
        pu.plot_trajectory_side(ax, p_gt_0, 'm', 'Ground Truth')
        plt.legend(bbox_to_anchor=(1, 1.45), loc=1, ncol=2, borderaxespad=0.)
        fig.tight_layout()
        fig.savefig(output_dir+'/'+dataset_nm +
